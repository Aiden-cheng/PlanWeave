...
describe.skipIf(!liveEnabled)("real ACP Host-local live smoke", () => {
  it("pref lights one Host-local profile, runs ACP-only execute + cancel, and records contract evidence", async () => {
    ...
    expect(
      evidence.preflight.agentVersion === null || evidence.preflight.agentVersion.length > 0
    ).toBe(true);
