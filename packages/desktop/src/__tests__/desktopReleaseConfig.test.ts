...
describe("desktop release configuration", () => {
  ...
  it("keeps platform verification fail closed and free of signing secrets", async () => {
    ...
    expect(packagedVerifier).toContain("startupReportFailure(report)");
...
`;
  ...
  it("keeps parallel test, performance, platform, and packaged gates explicit", async () => {
    ...
    expect(occurrenceCount(workflow, "cache: pnpm")).toBe(7);
    expect(occurrenceCount(workflow, "pnpm install --frozen-lockfile")).toBe(7);
