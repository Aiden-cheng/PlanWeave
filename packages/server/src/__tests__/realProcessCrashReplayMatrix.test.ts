...
describe("real-process crash/replay fault matrix", () => {
  it(
    "Host kill at ACP session/prompt: recovery does not auto-rerun ACP or false-complete",
    async () => {
  ...
  it(
    "Server kill during ACP barrier then restart: no false success; at most one ACP session",
    async () => {
      ...
      const view = await client.waitForOperation(
        ...
        (current) => {
          ...
          return ["failed", "cancelled", "interrupted", "action_required", "activated", "claimed"].includes(
            current.state
          );
        },
  ...
  it(
    "ACP force-exit mid-prompt fails closed without terminal success",
    async () => {
  ...
  it(
    "Server restart after terminal preserves completed state without replaying ACP",
    async () => {
      const { harness, client } = await createHarness();
      await harness.startAll();
      const dispatched = await client.dispatch({
  ...
  it(
    "capacity contention: one Host capacity=1 activates only one of two Blocks",
    async () => {
      const { harness, client } = await createHarness({
        hostCapacity: 1,
  ...
  it(
    "concurrent identical idempotency collapses to one logical remote operation",
    async () => {
  ...
  it(
    "cancel vs ACP barrier race settles cancelled without success",
    async () => {
      const { harness, client } = await createHarness();
      await harness.startAll();
      await harness.acpControl.pause(["session/prompt"]);
