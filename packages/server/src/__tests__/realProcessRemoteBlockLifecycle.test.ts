...
function assertIdentityChain(
  ...
  expected: { operationId: string; dispatchId: string; executionAttemptId: string; leaseId?: string }
): void {
...
describe("real-process remote Block lifecycle", () => {
  it(
    "success: eligibility → remote claim → host select → envelope/grant → WSS → ACP → report → Runtime submit",
    async () => {
  ...
  it(
    "ACP-declared failure (refusal) terminalizes with matching identities",
    async () => {
  ...
  it(
    "process/protocol error fails closed without false success",
    async () => {
  ...
  it(
    "user cancellation during ACP barrier reaches cancelled terminal with identity fidelity",
    async () => {
      const { harness, client } = await createHarness({ acpScenario: "success" });
      await harness.startAll();
  ...
  it(
    "interaction request/response (permission allow) then successful completion",
    async () => {
  ...
  it(
    "input/output artifacts and dependency summaries flow through envelope and Runtime",
    async () => {
  ...
  it(
    "event disconnect/replay preserves durable ACP events after Host restart",
    async () => {
      const { harness, client } = await createHarness({ acpScenario: "success" });
      await harness.startAll();
      await harness.acpControl.pause(["session/prompt"]);
  ...
  it(
    "two Hosts: capability selection prefers compatible Host; no-compatible-Host awaits capacity",
    async () => {
      const { harness, client } = await createHarness({
        acpScenario: "success",
  ...
  it(
    "local manual claim/submit works without Server",
    async () => {
