...
describe("real-process adversarial authorization matrix", () => {
  it(
    "operator and action identity fence matrix (wrong principal/scope/lease/version/schema/cursor)",
    async () => {
      ...
      const cases: MatrixCase[] = [
        ...
        },
        {
          ...
          run: async ({ client: c, operationId, dispatchId, executionAttemptId, leaseId, stateVersion }) => {
            const denied = await c.rawRequest({
              method: "POST",
              path: `/api/v1/remote-operations/${encodeURIComponent(operationId)}/actions`,
              body: {
  ...
  it(
    "action after terminal rejects further cancel without rewriting success",
    async () => {
      const { harness, client } = await createHarness();
      await harness.startAll();
      const dispatched = await client.dispatch({
  ...
  it(
    "foreign block ownership: second idempotency key cannot hijack active claim",
    async () => {
