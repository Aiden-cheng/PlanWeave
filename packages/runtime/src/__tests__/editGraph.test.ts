...
import {
  addEdge,
  addNode,
  commitPlanPackageGraphMutation,
  updatePromptSurface
...
describe("editGraph", () => {
  it("builds graph mutation previews with commit-only prompt side effects", async () => {
    ...
    const task = mutation.nextManifest.nodes.find(
    ...
    expect(mutation.sideEffects).toEqual([
      { kind: "removePrompt", packagePath: "nodes/T-001/blocks/R-001.prompt.md" }
    ]);
  });
  ...
  it("builds task and block package surface side effects in the graph mutation seam", async () => {
    const { init } = await createTestWorkspace();
    const manifest = await readJsonFile<PlanPackageManifest>(init.workspace.manifestFile);
    const task = basicManifest({ includeSecondTask: true }).nodes.find(
      (item) => item.type === "task" && item.id === "T-002"
    );
    ...
    expect(addTaskMutation.sideEffects).toEqual([
      ...
      {
        kind: "writePrompt",
        packagePath: "nodes/T-002/blocks/B-001.prompt.md",
        markdown: "# Implement second task\n"
      },
      {
        kind: "writePrompt",
        packagePath: "nodes/T-002/blocks/R-001.prompt.md",
        markdown: "# Review second task\n"
      }
    ]);
    ...
    const updatedTask = addBlockMutation.nextManifest.nodes.find(
      (node) => node.type === "task" && node.id === "T-001"
    );
    ...
    expect(addBlockMutation.sideEffects).toEqual([
      { kind: "writePrompt", packagePath: block.prompt, markdown: "# New block\n" }
    ]);
  });
  ...
  it("builds task and block field edits in the graph mutation seam", async () => {
    ...
    const updatedTask = taskMutation.nextManifest.nodes.find(
      (node) => node.type === "task" && node.id === "T-001"
    );
    if (updatedTask?.type !== "task") {
      throw new Error("Fixture task missing.");
    ...
    expect(
      updatedTask.blocks.every((block) => !("executor" in block) || block.executor === undefined)
    ).toBe(true);
    ...
    expect(blockMutation.updatedFields).toEqual([
      "title",
      "prompt",
      "executor",
    ...
    expect(blockMutation.sideEffects).toEqual([
      {
        kind: "writePrompt",
        packagePath: "nodes/T-001/blocks/B-001.prompt.md",
  ...
  it("removes task package directories through commit-only mutation side effects", async () => {
    ...
    expect(mutation.sideEffects).toEqual([
      { kind: "removeTaskDirectory", packagePath: "nodes/T-002" }
    ]);
    await expect(
      readFile(`${init.workspace.packageDir}/nodes/T-002/prompt.md`, "utf8")
    ).resolves.toContain("# T-002 task prompt");
  ...
  it("adds v1 task nodes with prompt source files and reports affected tasks", async () => {
    ...
    const node = basicManifest({ includeSecondTask: true }).nodes.find(
      (item) => item.type === "task" && item.id === "T-002"
    );
  ...
  it("validates edge endpoint contracts before writing", async () => {
    ...
    const result = await addEdge({
      projectRoot: root,
      edge: { from: "missing-task", to: "T-001", type: "depends_on" }
    });
  ...
  it("updates the task prompt source rather than a rendered task surface", async () => {
    ...
    const result = await updatePromptSurface({
      projectRoot: root,
      taskId: "T-001",
      taskBody: "# Updated task prompt\n"
    });

    expect(result.ok).toBe(true);
    await expect(
      readFile(`${init.workspace.packageDir}/nodes/T-001/prompt.md`, "utf8")
    ).resolves.toContain("# Updated task prompt");
  });
});
